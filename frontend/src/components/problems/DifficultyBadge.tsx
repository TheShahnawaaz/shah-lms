...
export const DifficultyBadge: React.FC<DifficultyBadgeProps> = ({ 
  ...
  if (variant === "detail") {
    return (
      <span className={`font-semibold px-2 py-0.5 rounded uppercase tracking-wider ${
        difficulty === 1 ? "bg-green-500/10 text-green-500" :
        difficulty === 2 ? "bg-yellow-500/10 text-yellow-500" : "bg-red-500/10 text-red-500"
      } ${className}`}>
  ...
  return (
    <span className={`px-2 py-0.5 rounded-md border ${getDifficultyColor(difficulty)} ${className}`}>
