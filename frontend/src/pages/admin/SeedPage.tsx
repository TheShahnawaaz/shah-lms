...
export const SeedPage: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        <h1 className="text-2xl font-semibold tracking-tight text-foreground">Database Seeder</h1>
        <p className="text-sm text-muted-foreground mt-1">Batch upload DSA problems from local JSON files.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-6">
          <div
            ...
            className="border-2 border-dashed border-border hover:border-primary bg-muted/10 hover:bg-muted/30 rounded-xl p-10 flex flex-col items-center justify-center gap-4 cursor-pointer transition-all group"
          >
            <input
              type="file"
              ref={fileInputRef}
            ...
            <div className="text-center">
              <p className="text-sm font-medium text-foreground">Drag & drop files or click to browse</p>
              <p className="text-xs text-muted-foreground mt-1">Supports multiple .json files</p>
            </div>
          </div>
            ...
            <div className="border border-border rounded-xl p-6 bg-card shadow-sm space-y-4">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="text-sm font-medium text-foreground">
                  Ready to Sync: <span className="text-primary">{selectedFiles.length} files</span>
                </div>
                <div className="flex items-center gap-2">
                  <label className="text-xs text-muted-foreground font-medium">Batch Size:</label>
                  <input
                    type="number"
                    value={batchSize}
                    onChange={(e) => setBatchSize(Math.max(1, parseInt(e.target.value) || 50))}
                    disabled={isSyncing}
                    className="w-20 bg-background border border-input rounded-md px-2 py-1.5 text-xs text-foreground focus:outline-none focus:ring-1 focus:ring-ring"
                  />
                </div>
              </div>
              ...
              {!isSyncing && progress === 0 ? (
                <button
                  ...
                  className="w-full py-2.5 rounded-md bg-primary hover:bg-primary/90 text-primary-foreground font-medium text-sm transition-colors flex items-center justify-center gap-2"
                >
                  <CheckCircle size={16} />
                  Start Sync
                </button>
                ...
                <div className="space-y-2 pt-2">
                  <div className="flex items-center justify-between text-xs font-medium">
                    <span className="text-primary truncate pr-4">{currentStep}</span>
                    <span className="text-foreground shrink-0">{progress}%</span>
                  </div>
        ...
        <div className="space-y-6">
          <div className="border border-border rounded-xl p-6 bg-card shadow-sm space-y-4">
            <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider">Sync Statistics</h3>
            
            <div className="space-y-2">
              {[
                { label: "Total Files", value: stats.totalFiles, color: "text-foreground" },
                { label: "Processed", value: stats.processedFiles, color: "text-primary" },
                { label: "Created", value: `+${stats.created}`, color: "text-emerald-500" },
                { label: "Updated", value: `~${stats.updated}`, color: "text-amber-500" },
                { label: "Skipped", value: stats.skipped, color: "text-muted-foreground" },
              ].map((stat, idx) => (
                <div key={idx} className="flex items-center justify-between p-2.5 rounded-md bg-muted/30 border border-border/50 text-sm">
          ...
          {errors.length > 0 && (
            <div className="border border-destructive/30 rounded-xl p-6 bg-destructive/5 space-y-3">
              ...
              <div className="max-h-48 overflow-y-auto text-xs space-y-2 font-mono scrollbar-thin scrollbar-thumb-destructive/20 pr-2">
                {errors.map((err, idx) => (
                  <div key={idx} className="p-2 rounded bg-background text-destructive/90 border border-destructive/20">
                    {err}
                  </div>
                ))}
              </div>
