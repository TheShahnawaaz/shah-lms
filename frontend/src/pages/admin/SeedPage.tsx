...
function md5(string: string) {
  ...
  function WordToHex(lValue: number) {
    ...
    for (lCount = 0; lCount <= 3; lCount++) {
      ...
      WordToHexValue =
        WordToHexValue + WordToHexValue_temp.substring(WordToHexValue_temp.length - 2);
    }
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-500">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Batch upload DSA problems from local JSON files.
        </p>
      </div>
          ...
          <div className="border border-border rounded-xl p-6 bg-card shadow-sm space-y-4">
            ...
            <div className="space-y-2">
              {[
                ...
                <div
                  key={idx}
                  className="flex items-center justify-between p-2.5 rounded-md bg-muted/30 border border-border/50 text-sm"
                >
            ...
            <div className="border border-destructive/30 rounded-xl p-6 bg-destructive/5 space-y-3">
              ...
              <div className="max-h-48 overflow-y-auto text-xs space-y-2 font-mono scrollbar-thin scrollbar-thumb-destructive/20 pr-2">
                {errors.map((err, idx) => (
                  <div
                    key={idx}
                    className="p-2 rounded bg-background text-destructive/90 border border-destructive/20"
                  >
                    {err}
                  </div>
                ))}
