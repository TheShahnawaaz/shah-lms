...
export const Layout: React.FC<LayoutProps> = ({ children, fullWidth = false }) => {
  ...
  const isImpersonating = localStorage.getItem("az_impersonated") === "true";

  const handleExitImpersonation = () => {
    const adminToken = localStorage.getItem("az_admin_token");
    const adminName = localStorage.getItem("az_admin_name");
    const adminEmail = localStorage.getItem("az_admin_email");
    const adminAvatar = localStorage.getItem("az_admin_avatar");

    if (adminToken) {
      localStorage.setItem("az_auth_token", adminToken);
      localStorage.setItem("az_user_name", adminName || "Admin");
      localStorage.setItem("az_user_email", adminEmail || "");
      if (adminAvatar) {
        localStorage.setItem("az_user_avatar", adminAvatar);
      } else {
    ...
    localStorage.removeItem("az_admin_name");
    localStorage.removeItem("az_admin_email");
    localStorage.removeItem("az_admin_avatar");
    localStorage.removeItem("az_impersonated");

    // Force reload to AllowedUsers control panel
  ...
  return (
    <div className="flex flex-col h-screen w-screen overflow-hidden bg-background text-foreground transition-colors duration-300 relative">
      ...
      {isImpersonating && (
        <div className="bg-amber-500/90 text-amber-950 px-4 py-2 flex items-center justify-between text-xs sm:text-sm font-semibold select-none border-b border-amber-600/20 shadow-sm z-50 shrink-0 backdrop-blur-sm">
          <div className="flex items-center gap-2">
            <span className="flex size-2 rounded-full bg-red-600 animate-pulse shrink-0" />
            <span className="truncate">
              Impersonation Mode: Exploring account of <strong>{userName}</strong> ({userEmail})
            </span>
          </div>
          <button
            onClick={handleExitImpersonation}
            className="px-3 py-1 bg-amber-950 text-amber-50 rounded-lg hover:bg-amber-900 transition-colors text-xs font-bold shrink-0 shadow-sm"
          >
            Exit Impersonation
          </button>
        </div>
