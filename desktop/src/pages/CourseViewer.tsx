...
const InlineMcqCard: React.FC<{ mcq: any }> = ({ mcq }) => {
  ...
  const blockMdComponents: any = {
    ...
    h2: ({ node, ...props }: any) => <h2 className="text-sm font-bold mt-1.5 mb-1" {...props} />,
    h3: ({ node, ...props }: any) => <h3 className="text-xs font-bold mt-1 mb-0.5" {...props} />,
  };
                              ...
                              >
                                <ReactMarkdown
                                  ...
                                  components={{
                                    ...
                                    img: ({ node, ...props }) => (
                                      <span className="flex justify-center my-8 select-none">
                                        <span className="bg-white dark:bg-card p-3 rounded-2xl border border-border/80 dark:border-border/30 shadow-md hover:shadow-xl dark:shadow-primary/5 dark:hover:shadow-primary/15 hover:-translate-y-0.5 transition-all duration-300 max-w-full md:max-w-[85%] inline-block">
                                          <img
                                    ...
                                    h1: ({ node, ...props }) => (
                                      <h1 className="text-xl font-bold mt-5 mb-3 text-foreground border-b border-border pb-1" {...props} />
                                    ),
                                    h2: ({ node, ...props }) => (
                                      <h2 className="text-lg font-bold mt-4 mb-2 text-foreground" {...props} />
                                    ),
                                    h3: ({ node, ...props }) => (
                                      <h3 className="text-base font-bold mt-3 mb-1 text-foreground" {...props} />
                                    ),
                                    p: ({ node, ...props }) => <p className="mb-4 text-foreground leading-relaxed" {...props} />,
                                    ul: ({ node, ...props }) => <ul className="list-disc pl-5 mb-4 space-y-1.5 text-foreground" {...props} />,
                                    ol: ({ node, ...props }) => <ol className="list-decimal pl-5 mb-4 space-y-1.5 text-foreground" {...props} />,
